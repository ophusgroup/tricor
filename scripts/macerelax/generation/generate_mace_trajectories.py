...
# --- Per-CIF generation ---
N_TRAJ_PER_CIF = 1              # matches relaxml-big convention
BASE_SEED      = 1_000_000
...
REGIME_STRATA = ["liquid", "amorphous", "SRO", "MRO", "LRO",
...
def build_configs(n_traj: int, base_seed: int) -> list[SampleConfig]:
    ...
    At the default n_traj=20 with 7 regimes the distribution is
    [3, 3, 3, 3, 3, 3, 2] — 3 trajectories each across the first 6
    densest regimes, 2 for liquid (least dense).  Adequate stratification
    without per-regime quotas.
    """
