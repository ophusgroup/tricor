...
# --- Output ---
# MRO-V2 FORK: corrects the MRO regime's relative_density from 0.88 (a physical
# outlier baked into big_exp — MRO is more ordered than SRO/amorphous yet was
# the LEAST dense) to 0.92, consistent with the other disordered regimes.
# Writes to a SEPARATE dataset root so big_exp stays immutable; at training-
# assembly time MRO trajectories come from here (idx=5 @0.92), NOT from
# big_exp's superseded idx=5 @0.88.
DATASET_ROOT  = Path("/pscratch/sd/e/ehrdt/tricor/big_exp_mro_v2")

# --- Per-CIF generation ---
# MRO-V2 FORK: one corrected MRO trajectory per CIF at idx=5 (MRO's slot in
# big_exp), reusing the SAME seed (base + 5) so the only difference vs the
# original is relative_density 0.88 -> 0.92.  Deterministic + reproducible.
N_TRAJ_PER_CIF = 1
IDX_OFFSET     = 5              # MRO lives at idx00005 / seed = base + 5
...
SOURCE_TAG     = "mace_mro_v2"  # written into every NPZ's `source` field

# --- Calibration reuse ---
# MRO-V2 FORK: load the per-CIF calibrated cell sizes from the ORIGINAL big_exp
# run so corrected MRO uses the EXACT same cell as that CIF's other regimes and
# skips the OOM probe.  0.92 < crystalline_30's 0.98, so it fits those cells.
...
# MRO-V2 FORK: only the MRO regime, regenerated at the corrected density.
REGIME_STRATA = ["MRO"]
DENSITY_BY_REGIME = {
    # Corrected from big_exp's 0.88 (an inversion: MRO has larger grains /
    # more order than SRO 0.92 and amorphous 0.92, so it should be at least as
    # dense, not the least dense).  0.92 flats it with the other disordered
    # regimes for a monotonic density-vs-order ladder.
    "MRO":              0.92,
}
