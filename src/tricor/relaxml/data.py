"""Data loading for the relaxml surrogate.

Reads .npz trajectories + manifest.csv produced by
``scripts/flowmatch_guided/generate_surrogate_trajectories.py``.  Each
training example is a consecutive-snapshot pair ``(x_t, x_{t+1})``
drawn from one trajectory; target is the min-image displacement
``x_{t+1} - x_t``.

With ``k_stride_snapshots = 1`` and our default trajectory stride of 5,
each trajectory contributes (num_snapshots - 1) pairs — ~40 per config,
~80k total across a 2000-config dataset.
"""
