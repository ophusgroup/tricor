"""Data loading for the shell_target-conditioned relaxml surrogate.

Variant of ``data.py`` that also reads four shell_target arrays per
.npz (written by the multi-species generator after the shell_target
extension, or backfilled by ``scripts/relaxml/add_shell_target_to_npz.py``)
and emits them as Data attributes consumable by ``model_shelltgt``.

Per-graph shell_target arrays are batched by PyG using the
``ShellTargetData`` subclass below — it overrides ``__cat_dim__`` so the
variable-length per-pair / per-triplet attributes concatenate along
dim 0, and exposes ``shell_pair_batch`` / ``shell_trip_batch`` as
node-style index tensors that get the per-graph offset PyG would
otherwise compute for ``edge_index``.
"""
...
# network sees values in roughly [0, ~1.5].  Scales chosen to match the
# sampling ranges in scripts/flowmatch_guided/diagnose_param_space.py
# and generate_surrogate_trajectories.py.
...
def _weight_vector_from_row(row: dict, num_grains: int | None = None) -> np.ndarray:
    """Build the normalized (NUM_WEIGHT_FEATURES,) global vector from a manifest row."""
    if num_grains is None:
        num_grains = int(float(row["num_grains"]))
    vals = {
        "bond_weight":          float(row["bond_weight"]),
        "angle_weight":         float(row["angle_weight"]),
        "repulsion_weight":     float(row["repulsion_weight"]),
        "hard_core_scale":      float(row["hard_core_scale"]),
        "nonbond_push_scale":   float(row["nonbond_push_scale"]),
        "displacement_sigma":   float(row["displacement_sigma"]),
