"""Step-by-step relaxation surrogate, with shell_target phase conditioning.

Variant of ``model.py`` that adds explicit phase conditioning: each
graph carries a flattened ``CoordinationShellTarget`` (per-pair targets:
distance, sigma, coordination numbers; per-triplet targets: angle mode,
weight) which is encoded as a deep set and added to the global per-graph
conditioning vector alongside the existing 9-D regime weight features.

This lets one model serve different phases of the same compound (e.g.
α-quartz vs β-cristobalite SiO₂) by changing the shell_target input
without retraining.
"""
