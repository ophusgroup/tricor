...
function makeColor(value, vmin, vmax, midpoint = null) {
  let t = clamp((value - vmin) / Math.max(vmax - vmin, 1e-12), 0, 1);
  const stops = [
    [0.0, [19, 53, 97]],
    [0.2, [61, 122, 176]],
    [0.4, [150, 198, 226]],
...
function drawHeatmap(canvas, imageValues, shape, rEdges, phiEdgesDeg, title, normalize, vmax, autoContrast) {
  ...
  const height = Math.max(Math.round(canvas.clientHeight || 300), 230);
  const margin = { top: 30, right: 92, bottom: 42, left: 74 };
  ...
  ctx.save();
  ctx.translate(bar.x + bar.width + 32, bar.y + bar.height / 2);
  ctx.rotate(-Math.PI / 2);
  ctx.textAlign = "center";
...
function drawPairCompare(svg, superProfile, targetProfile, rValues, rEdges, selection, dragging) {
  const width = Math.max(Math.round(svg.clientWidth || 860), 680);
  const height = Math.max(Math.round(svg.clientHeight || 220), 170);
  const margin = { top: 18, right: 92, bottom: 42, left: 74 };
  const plotWidth = width - margin.left - margin.right;
  const plotHeight = height - margin.top - margin.bottom;
  const yMax =
    Math.max(Math.max(...superProfile, ...targetProfile, 1e-12) * 1.05, 1.1);
  const xMax = Math.max(rEdges.length ? rEdges[rEdges.length - 1] : 1, 1e-12);

  const xScale = (value) => margin.left + (value / xMax) * plotWidth;
  const yScale = (value) => margin.top + plotHeight - (value / yMax) * plotHeight;
  const toPoints = (profile) =>
    profile.map((value, idx) => `${xScale(rValues[idx])},${yScale(value)}`).join(" ");
  ...
  svg.innerHTML = `
    <rect class="compare-panel-bg" x="0" y="0" width="${width}" height="${height}"></rect>
    <text class="compare-panel-title" x="${margin.left}" y="12">2 body comparison</text>
    <rect class="compare-selection-area" x="${Math.min(areaX0, areaX1)}" y="${margin.top}" width="${Math.abs(areaX1 - areaX0)}" height="${plotHeight}"></rect>
    <line class="compare-ideal-line" x1="${margin.left}" x2="${margin.left + plotWidth}" y1="${yScale(1.0)}" y2="${yScale(1.0)}"></line>
    <text class="compare-ideal-label" x="${margin.left + 6}" y="${yScale(1.0) - 6}" text-anchor="start">g(r)=1</text>
    <polyline class="compare-target-line" points="${toPoints(targetProfile)}"></polyline>
    <polyline class="compare-super-line" points="${toPoints(superProfile)}"></polyline>
    <line class="compare-selection-handle ${dragging === "min" ? "dragging" : ""}" x1="${areaX0}" x2="${areaX0}" y1="${margin.top}" y2="${margin.top + plotHeight}"></line>
    <line class="compare-selection-handle ${dragging === "max" ? "dragging" : ""}" x1="${areaX1}" x2="${areaX1}" y1="${margin.top}" y2="${margin.top + plotHeight}"></line>
    <rect class="compare-plot-frame" x="${margin.left}" y="${margin.top}" width="${plotWidth}" height="${plotHeight}"></rect>
    <text class="compare-axis-label" x="${margin.left + plotWidth / 2}" y="${height - 8}">r</text>
    <text class="compare-axis-label" x="18" y="${margin.top + plotHeight / 2}" text-anchor="middle" transform="rotate(-90 18 ${margin.top + plotHeight / 2})">g(r)</text>
    <line class="compare-legend-swatch compare-target-swatch" x1="${width - 78}" x2="${width - 48}" y1="${margin.top + 14}" y2="${margin.top + 14}"></line>
    <text class="compare-legend-label" x="${width - 42}" y="${margin.top + 18}">target</text>
    <line class="compare-legend-swatch compare-super-swatch" x1="${width - 78}" x2="${width - 48}" y1="${margin.top + 34}" y2="${margin.top + 34}"></line>
    <text class="compare-legend-label" x="${width - 42}" y="${margin.top + 38}">supercell</text>
  `;
  ...
  xTickValues.forEach((value) => {
    ...
    tick.setAttribute("class", "compare-axis-tick");
    ...
    label.setAttribute("class", "compare-tick-label");
...
function render({ model, el }) {
  ...
  root.appendChild(supercellPanel);
  ...
  function redraw() {
    ...
    const superPairProfile = model.get("supercell_pair_profile") || [];
    const targetPairProfile = model.get("target_pair_profile") || [];
    const superSlice = model.get("supercell_slice_image") || [];
    const targetSlice = model.get("target_slice_image") || [];
    const sliceShape = model.get("slice_shape") || [0, 0];
    selection.min = model.get("selection_min");
    selection.max = model.get("selection_max");

    const combinedFinite = [...superSlice, ...targetSlice].filter((value) => Number.isFinite(value));
    const autoVmax = Math.max(
      percentile(combinedFinite, 0.995) || 1,
      checkbox.checked ? 1.001 : 1e-6
    );
    ...
    overlay.setAttribute("class", "compare-drag-overlay");
  ...
  model.on(
    "change:triplet_labels change:triplet_index change:normalize change:sigma_r change:sigma_phi change:slice_max change:supercell_pair_profile change:target_pair_profile change:supercell_slice_image change:target_slice_image change:slice_shape change:selection_min change:selection_max change:status change:r_edges change:phi_edges_deg change:supercell_title change:target_title",
    redraw
  );
